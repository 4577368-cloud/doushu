...
import { Crown, Compass, Edit2, FileText, Trash2, X, Tag, Activity, LogOut, User, PlusCircle } from 'lucide-react';
...
export const ArchiveView: React.FC<{ archives: UserProfile[]; setArchives: any; onSelect: any; isVip: boolean; onVipClick: () => void; session: any; onLogout: () => void }> = ({ archives, setArchives, onSelect, isVip, onVipClick, session, onLogout }) => {
    ...
    const toggleTag = (tag: string) => { if (!editingProfile) return; const currentTags = editingProfile.tags || []; const newTags = currentTags.includes(tag) ? currentTags.filter(t => t !== tag) : [...currentTags, tag]; setEditingProfile({ ...editingProfile, tags: newTags }); };
    const addCustomTag = () => { if (!customTag.trim() || !editingProfile) return; const currentTags = editingProfile.tags || []; if (!currentTags.includes(customTag.trim())) { setEditingProfile({ ...editingProfile, tags: [...currentTags, customTag.trim()] }); } setCustomTag(''); };

    // 创建"我"的档案逻辑：跳转回首页，并让用户排盘，最好是能直接把首页表单的 isSelf 设为 true，这里简单处理为让用户去排盘并在保存时引导
    // 更好的方式：弹出一个简化表单直接创建。为了复用首页强大的排盘，我们这里做一个简单的引导。
    const handleCreateMe = () => {
        alert("请前往【首页】输入您的生辰信息进行排盘。\n\n⚠️ 重要：在排盘结果页点击【保存档案】后，\n请到【档案】页点击该卡片编辑，勾选【设为本人】。");
        // 这里逻辑稍微有点绕，因为保存是在 ChartView。
        // 优化方案：我们在编辑弹窗里加一个 "设为本人" 的开关。
    };
        ...
        <div className="h-full flex flex-col bg-[#f5f5f4] overflow-y-auto pb-24">
             {session && (
                 <div className="bg-white border-b border-stone-200 px-5 py-4 flex items-center justify-between sticky top-0 z-50 shadow-sm">
                     <div className="flex items-center gap-3"><div className="w-10 h-10 rounded-full bg-stone-900 text-amber-500 flex items-center justify-center font-bold text-lg border-2 border-amber-500 shadow-sm">{session.user.email?.[0].toUpperCase()}</div><div><p className="text-xs font-bold text-stone-900">{session.user.email}</p><p className="text-[10px] text-stone-400 font-medium">云端同步已开启</p></div></div>
                     <button onClick={onLogout} className="p-2 bg-stone-50 text-stone-500 rounded-lg hover:bg-stone-100 border border-stone-200"><LogOut size={16} /></button>
                 </div>
             )}
                            ...
                            <div className="relative z-10">
                                <div className="flex justify-between items-start gap-4 mb-4">
                                    <div className="flex-1">
                                        <div className="flex items-center gap-2 mb-1"><h3 className="font-black text-white text-xl">{myProfile.name}</h3><span className="text-[10px] px-2 py-0.5 rounded-full font-bold bg-amber-500 text-stone-900">本人</span></div>
            ...
            {viewingReports && <div className="fixed inset-0 z-[1100] flex items-center justify-center p-4"><div className="absolute inset-0 bg-stone-900/60 backdrop-blur-md" onClick={() => setViewingReports(null)} /><div className="relative bg-white w-full max-w-lg rounded-[2.5rem] shadow-2xl flex flex-col max-h-[85vh] animate-slide-up overflow-hidden"><div className="p-5 border-b border-stone-100 flex justify-between items-center bg-stone-50/50"><h3 className="font-black text-stone-900">{viewingReports.name} 的报告库</h3><X onClick={() => setViewingReports(null)} size={20} className="text-stone-400 cursor-pointer"/></div><div className="flex-1 overflow-y-auto p-5 space-y-4 custom-scrollbar">{viewingReports.aiReports?.length ? viewingReports.aiReports.map(r => (<div key={r.id} className="bg-white border border-stone-200 rounded-2xl p-4 shadow-sm space-y-2"><div className="flex justify-between items-center"><span className="text-[10px] font-black px-1.5 py-0.5 rounded-full bg-amber-50 text-amber-700 border border-amber-100">{r.type==='ziwei'?'紫微':'八字'}</span><span className="text-[9px] text-stone-400">{new Date(r.date).toLocaleString()}</span></div><div className="text-[12px] text-stone-700 leading-relaxed whitespace-pre-wrap font-medium">{typeof r.content === 'string' ? r.content : JSON.stringify(r.content, null, 2)}</div><button onClick={()=>{navigator.clipboard.writeText(String(r.content)); alert('已复制');}} className="w-full py-2 bg-stone-100 text-stone-700 rounded-xl text-[10px] font-bold">复制全文</button></div>)) : <div className="text-center py-20 text-stone-300 italic">暂无记录</div>}</div></div></div>}
        </div>
    );
};